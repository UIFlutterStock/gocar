...
class IniciarCorridaWidget extends StatelessWidget {
  GlobalKey<ScaffoldState> scaffoldKey;

  IniciarCorridaWidget(this.scaffoldKey);
  ...
  Future<void> _iniciarViagem(BuildContext context) async {
    ...
    if (distanciaTempo.Distancia.contains('km')) {
      ...
      if (distancia > 1) {
        ShowSnackBar.build(
            scaffoldKey,
            'Desculpe a viagem ainda não pode ser iniciada você está a $distancia km do local de embarque .',
            context);
        return false;
      }
    }

    viagem.Status = StatusViagem.Iniciada;
    await _viagemService.save(viagem);
    _homeBloc.stepMotoristaEvent.add(StepMotoristaHome.Iniciarviagem);
