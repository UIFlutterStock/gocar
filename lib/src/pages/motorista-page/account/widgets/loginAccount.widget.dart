...
class _LoginAccountState extends State<LoginAccount> {
  ...
  void _functionLoginWithEmailPass(BuildContext context) {
    if (_formKey.currentState.validate()) {
      ...
      _auth
        ...
        NavigationPagesMotorista.goToHome(context);
      }, onError: (ex) {
        new Future.delayed(new Duration(seconds: 1), () {
          widget._baseBloc.loadingStatusEvent.add(false);
        });
        ...
        switch (ex.code) {
          case "ERROR_WRONG_PASSWORD":
            ShowSnackBar.build(widget._scaffoldLoginKey,
                'Usuáro ou Password inválido!', context);
            break;
          ...
          default:
            ShowSnackBar.build(widget._scaffoldLoginKey,
                'Desculpas, ocorreu um problema!', context);
            break;
        }
      });
    }
  ...
  void _functionLoginWithGoogle(BuildContext context) {
    ...
    _auth.signWithGoogle().then((r) {
      ...
      switch (ex.code) {
        case "ERROR_WRONG_PASSWORD":
          ShowSnackBar.build(
              widget._scaffoldLoginKey, 'Usuário ou senha incorreta!', context);
          break;
        case "ERROR_INVALID_EMAIL":
          ShowSnackBar.build(
              widget._scaffoldLoginKey, 'Email invalido!', context);
          break;
        default:
          ShowSnackBar.build(
              widget._scaffoldLoginKey, 'Ocorreu um erro desconhecido !',
              context);
          break;
      }
    });
  ...
  Widget _buildForgot() => Padding(
        ...
        child: FlatButton(
            onPressed: () =>
                NavigationPagesMotorista.goToRecoveryPass(context),
            child: Text(
              "Esqueçeu a Senha?",
              style: TextStyle(
            ...
            Row(
              ...
              children: <Widget>[
                _buildFacebookGoogleButton(
                    ...
                    ()  {
                      ShowSnackBar.build(widget._scaffoldLoginKey, 'Atualmente somente é possivel realizar login com conta Google ou com E-mail!', context);
                      return false;
                    },
