...
class StepMotoristaProcurarViagem {
  ViagemService _viagemService = ViagemService();
  HomeMotoristaBloc _homeMotoristaBloc =
      BlocProvider.getBloc<HomeMotoristaBloc>();
  AuthMotoristaBloc _authBloc = BlocProvider.getBloc<AuthMotoristaBloc>();
  BaseMotoristaBloc _baseMotoristaBloc =
      BlocProvider.getBloc<BaseMotoristaBloc>();
  ...
  StepMotoristaInicioBiz _stepMotoristaInicioBiz = StepMotoristaInicioBiz();
  bool indicaStatusProcesso = true;
  StreamSubscription<Position> _streamPositionInicial;


  static StepMotoristaProcurarViagem _instance;

  factory StepMotoristaProcurarViagem() {
    _instance ??= StepMotoristaProcurarViagem._internalConstructor();
    return _instance;
  }

  StepMotoristaProcurarViagem._internalConstructor();
    ...
    encerrarFluxosPosicaoLocalMotorista();
      ...
      _streamSpecificAbertaViagem = streamViagemEspecifica.listen((data) {
        data.documentChanges.forEach((changeResult) async {
          ...
          if (viagemEspefica.Status == StatusViagem.MotoristaACaminho) {
            ...
            if (motorista.Id == viagemEspefica.MotoristaEntity.Id) {
              ...
              await visualizaLocalizacaoPassageiroMotorista(
                  viagemEspefica, StatusViagem.MotoristaACaminho);
              /*atualiza a tela*/
              _homeMotoristaBloc.stepMotoristaEvent
                  .add(StepMotoristaHome.Viagemaceita);
              ...
              encerraFluxo();
              _homeMotoristaBloc.stepMotoristaEvent
                  .add(StepMotoristaHome.Procurandoviagem);
              Start();
            }
            /*fecha o modal pois a viagem foi iniciada */
          } else if (viagemEspefica.Status == StatusViagem.Cancelada) {
            /*mata  o stream evitar encalar os processos*/
            encerraFluxo();
            _homeMotoristaBloc.stepMotoristaEvent
                .add(StepMotoristaHome.Procurandoviagem);
  ...
  Future<void> adicionaLocalizacaoInicialPassageiroMotorista(
    ...
    _homeMotoristaBloc.stepMotoristaEvent
        .add(StepMotoristaHome.Viagemencontrada);
    /*adiciona monitoramento */
    Future.delayed(const Duration(milliseconds: 500), () async {
      await visualizaLocalizacaoPassageiroMotorista(
          viagem, StatusViagem.MotoristaNotificado);
    });
  }

  Future<void> visualizaLocalizacaoPassageiroMotorista(
      Viagem viagem, StatusViagem statusViagem) async {
    var locationOptions =
    ...
    _streamPosition = _geolocator
      ...
      if (position != null && semaforo) {
        ...
        Future.delayed(const Duration(milliseconds: 1800), () async {
          ...
          if (statusViagem == StatusViagem.MotoristaACaminho) {
